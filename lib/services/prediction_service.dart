...
class PredictionService {
  ...
  CyclePhase get currentPhase {
    ...
    if (latestPeriod.endDate != null) {
      isStillBleeding = !normToday.isAfter(
        DateTime(
          latestPeriod.endDate!.year,
          latestPeriod.endDate!.month,
          latestPeriod.endDate!.day,
        ),
      );
    } else {
    ...
    } else if (daysSinceStart >= ovulationDay - 5 &&
        daysSinceStart <= ovulationDay) {
  ...
  bool isFertileDay(DateTime date) {
    ...
    final fertileStart = latestPeriod.startDate.add(
      Duration(days: ovulationDay - 5),
    );
    final fertileEnd = latestPeriod.startDate.add(
  ...
  int getConceptionChance(DateTime date) {
    ...
    final diff =
        daysSinceStart - ovulationDay + 1; // Adjust to match cycle day logic
